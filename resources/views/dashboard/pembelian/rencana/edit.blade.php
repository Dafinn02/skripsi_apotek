        ...
        <div class="row">
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              ...
              <!-- /.card-header -->
              <!-- form start -->
              <form action="{{url('/master/produk/update/'.$data->id)}}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="card-body">
                  <div class="row">
                      ...
                      <div class="form-group">
                       ...
                       <select name="unit_id" class="select2" style="width: 100%; height: 100%" >
                         ...
                         @foreach($unit as $uKey => $uItem)
                           <option value="{{$uItem->id}}" {{$data->unit_id == $uIitem->id ? 'selected':''}}>
                            {{$uItem->name}}
                          </option>
                         @endforeach
                       </select>
                      ...
                      <div class="form-group">
                       ...
                       <select name="supplier_id" class="select2" style="width: 100%; height: 100%" >
                         ...
                         @foreach($supplier as $sKey => $sItem)
                           <option value="{{$sItem->id}}" {{$data->supplier_id == $sItem->id ? 'selected':''}}>
                            {{$sItem->name}}
                          </option>
                         @endforeach
                       </select>
